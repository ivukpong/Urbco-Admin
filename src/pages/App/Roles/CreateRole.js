  ...
  const actionHandler = async (e) => {
    ...
    try {
      ...
      if (details._id) {
        ...
        res = await userService.createRole(user_details.access_token, data);
      }
      ...
      if (res) {
        onComplete();
        onCancel();
        toast.success(`Role has been ${details._id ? "Edited" : "Added"}`, {
          position: "top-right",
        });
      }
    } catch (err) {
          ...
          permissions.map((p) => (
            <div className="check" key={p._id}>
              <input
                type="checkbox"
                checked={
                  permissionsList.find((l) => l === p.code) ? true : false
                }
