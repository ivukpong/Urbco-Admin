  ...
  const actionHandler = async (e) => {
    ...
    try {
      ...
      if (details._id) {
        ...
        res = await featureService.createTypes(user_details.access_token, data);
      }
      ...
      if (res) {
        ...
        onCancel();
        toast.success(
          `Property types has been ${details._id ? "Edited" : "Added"}`,
          {
            position: "top-right",
          },
        );
    ...
    <form className="form" onSubmit={actionHandler}>
      ...
      {details._id && (
        <div className="text-end mb-3">
          <button
            type="button"
            className="desc btn btn-link p-0"
            onClick={() => setStatusScreen(!statusScreen)}
          >
            {statusScreen
              ? "Go Back"
              : status === "active"
